  ...
  describe '配送先情報の保存' do
    ...
    context '配送先情報が保存できない場合' do
      ...
      it '郵便番号が「3桁-4桁」の半角文字列でなければ保存できないこと' do
        ...
        expect(@form).to_not be_valid
        expect(@form.errors.full_messages).to include("Postal code は「3桁-4桁」の半角文字列で入力してください")
      end
      ...
      it '電話番号が10桁未満では保存できないこと' do
        ...
        expect(@form).to_not be_valid
        expect(@form.errors.full_messages).to include("Tell number は10桁以上11桁以内の半角数値で入力してください")
      end
